...
(defrecord NewContactKey [sym-key message]
  ...
  (send [this cofx chat-id]
    (protocol/send-with-pubkey {:web3    web3
                                :chat-id chat-id
                                :payload this}))
  (receive [this cofx chat-id]
    (let [{:keys [web3]} (:db cofx)]
      {:shh/add-new-sym-key {:sym-key    sym-key
                             :chat-id    chat-id
                             :message    message
...
(defrecord ContactRequest [name profile-image address fcm-token]
  ...
  (receive [this cofx chat-id signature]
    (let [message-id (protocol/message-id message)]
...
(handler/reg-event-fx
 ...
 (fn [{:keys [db]} [_ {:keys [sym-key-id chat-id message]}]]
   (let [cofx {:db (assoc-in db [:transport/chats chat-id :sym-key-id] sym-key-id)}]
     (message/receive message cofx chat-id signature))))
